...
# ------------------------------
def cross_validate_ner(tokens, labels, cv=5):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X_vectorized), start=1):
        ...
        print(classification_report(y_test, y_pred, labels=[l for l in set(y_test) if l != 'O']))
...
if __name__ == "__main__":
    ...
    cross_validate_ner(tokens, labels_split, cv=10)

    # Spojene label-e
    labels_merged = combined_df['label'].apply(merge_labels).tolist()
    cross_validate_ner(tokens, labels_merged, cv=10)
